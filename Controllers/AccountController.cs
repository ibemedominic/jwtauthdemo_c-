...
namespace JwtAuthDemo.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<ActionResult> RefreshToken([FromBody] RefreshTokenRequest request)
        {
            try
            {
                if(!ModelState.IsValid)
                {
                    ...
                    //this.Response.StatusCode = Microsoft.AspNetCore.Http.StatusCodes.Status400BadRequest;
                    //Problem(serial)
                    return BadRequest();
                }
                //var userName = User.Identity?.Name;
                ...
                var jwtResult = _jwtAuthManager.Refresh(request.RefreshToken, accessToken, DateTime.Now, true);
        ...
        public ActionResult Impersonate([FromBody] ImpersonationRequest request)
        {
            ...
            var jwtResult = _jwtAuthManager.GenerateTokens(request.UserName, claims, DateTime.Now);
            _logger.LogInformation($"User [{userName}] is impersonating [{request.UserName}] in the system.");
            return Ok(new LoginResult
            {
                UserName = request.UserName,
