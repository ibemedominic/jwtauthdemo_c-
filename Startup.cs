...
namespace JwtAuthDemo
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            RsaSecurityKey privateEncryptionKey = new RsaSecurityKey(encryptionKey) { KeyId = JwtTokenConfig.EncryptionKid };
            RsaSecurityKey publicEncryptionKey = new RsaSecurityKey(encryptionKey.ExportParameters(false)) { KeyId = JwtTokenConfig.EncryptionKid };
            ECDsaSecurityKey privateSigningKey = new ECDsaSecurityKey(signingKey) { KeyId = JwtTokenConfig.SigningKid };
            ECDsaSecurityKey publicSigningKey = new ECDsaSecurityKey(ECDsa.Create(signingKey.ExportParameters(false))) { KeyId = JwtTokenConfig.SigningKid };
            ...
            services.AddSingleton<IJwtAuthManager, JwtAuthManager>((a)=>
            {
                return new JwtAuthManager(jwtTokenConfig, privateEncryptionKey, publicEncryptionKey, privateSigningKey, publicSigningKey);
            });
